...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable() //отключаем защиту от межсайтовой подделки запросов (Временно отключаем отправку токена)
                ...
                .and()
                .logout().logoutUrl("/lodout")// Тот Url по которому будет производиться logout (Logout - это когда из сессии удаляется пользователь и у пользователя в браузере удаляются Cookies после этого пользователь становиться не аутентифицирован)
